...
(function () {
  ...
  function setLang(v) {
    ...
    render();
    if (state.project) openProject(state.project, true, "replace");
  }
  ...
  function render() {
    ...
    /* A render never changes the route — it repaints the one we are
       already on. Writing here would clobber a deep link before
       fromHash has had a chance to read it. */
    showView(state.view, true, "none");
  }

  function showView(id, silent, hist) {
    state.view = id;
  ...
  /* "push"    — the visitor went somewhere: leave a trail
     "replace" — same place, new URL shape: overwrite
     "none"    — we are here *because* of the history: write nothing */
  function mark(hash, hist) {
    if (hist === "none") return;
    if (location.hash === hash) return;
    if (hist === "push") history.pushState(null, "", hash);
  ...
  function wipeTo(id, origin, worldVar) {
    if (reduced) { showView(id, false, "push"); return; }
    ...
    setTimeout(function () { showView(id, false, "push"); }, 150);
    setTimeout(function () { v.classList.remove("is-wiping"); }, 940);
  }

  /* ---------- project view -------------------------------- */
  function openProject(id, silent, hist) {
    state.project = id;
    ...
    mark("#/creator/" + id, hist);
  }

  function closeProject(hist) {
    state.project = null;
  ...
  /* ---------- routing ------------------------------------- */
  /* Runs on boot and on every Back/Forward. It reports what the URL
     already says, so it writes no history of its own, and it touches
     only what actually differs — otherwise Back out of a project
     would tear down and rebuild the world behind it. */
  function fromHash() {
    var h = (location.hash || "").replace(/^#\/?/, "").split("/").filter(Boolean);
    var view = h[0] || "hub";
    if (MW.worlds.indexOf(view) === -1) view = "hub";

    var proj = (view === "creator" && h[1] &&
                MW.projectOrder.indexOf(h[1]) !== -1) ? h[1] : null;

    if (view !== state.view) showView(view, true, "none");
    if (proj !== state.project) {
      if (proj) openProject(proj, true, "none");
      else closeProject("none");
    }
  }
  ...
  function wire() {
    document.addEventListener("click", function (e) {
      ...
      if (proj && proj.tagName === "BUTTON") {
        ...
        if (pop(proj)) setTimeout(function () { openProject(pid, false, "push"); }, 260);
        else openProject(pid, false, "push");
        return;
      }
    ...
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape") {
        if (state.project) closeProject("push");
        else if (state.view === "hub" && reachIsOpen()) setReach(false);
        else if (state.view !== "hub") wipeTo("hub", null, "var(--accent)");
      }
    });

    /* A fragment-only history move fires both; fromHash is a no-op
       when nothing differs, so handling both is safe and means we
       never miss one. */
